...
from sub import base_features_gen_pipeline, gen_meta_features, test_prediction
...
# Load data
train_data = spark.read.load("F:/9313/COMP9313_project_2/proj2train.csv", format="csv", sep="\t", inferSchema="true", header="true")
test_data = spark.read.load("F:/9313/COMP9313_project_2/proj2test.csv", format="csv", sep="\t", inferSchema="true", header="true")
start_time = time()
...
training_set.select("group").distinct().show()
...
print(meta_features.count())
meta_features.printSchema()

meta_features.select("nb_pred_0","svm_pred_0","joint_pred_0").show(50)
...
pred_test.select("id","label","final_prediction").show()
# Evaluation
